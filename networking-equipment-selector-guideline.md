...
- **UX modes are three renderings of the same response** — the contract came first, then the modes cost little. *(Done 2026-07: the web UI ships a mode-choice landing plus `#lookup` (exact-model option tables), `#full` (all facet controls, structured kitlists, raw JSON only behind a button), and `#guided` (tile-based steps in the registry's `ask_priority` order, with `presentation.depends_on` gating dependent questions — technology after stacking, cables after technology, triple after redundant, module after modular — ending in an inline kitlist plus an optional jump to the pre-filled full view). A question-based conversational mode is deliberately NOT a fourth UI — that is the Stage-3 agent through MCP.)*
